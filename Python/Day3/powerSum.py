...
def power_sum(arr, depth=1):
    ...
    for element in arr:
        if isinstance(element, list):
            total += power_sum(element, depth + 1)
        else:
            total += element
    return total ** depth

def powerSum(arr, depth=1):
    sum = 0
    for element in arr:
        if type(element) == list:
            sum += powerSum(element, depth + 1)
        else:
            sum += element
    return sum ** depth

# Example usage:
peculiar_array1 = [2, 3, [4, 1, 2]]
peculiar_array2 = [1, 2, [7, [3, 4], 2]]
print(power_sum(peculiar_array1))
print(power_sum(peculiar_array2))